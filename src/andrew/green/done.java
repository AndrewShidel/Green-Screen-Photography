...
public class done extends Activity
{
	...
	public void set(View view){
		try
		{
			...
			Toast.makeText(getBaseContext(), "Background Set", Toast.LENGTH_SHORT).show(); 
		}
		catch (IOException e){
			Toast.makeText(getBaseContext(), "Background could not be set", Toast.LENGTH_SHORT).show(); 
		}
	}
	...
	public void fb(View view){
		...
		/*Intent intent=new Intent(Intent.ACTION_SEND,Uri.parse("https://www.facebook.com"));
		intent.putExtra("sms_body", "Sent from Green Screen Photography.");
		intent.putExtra(Intent.EXTRA_STREAM, Uri.fromFile(new File(s)));
		...
		if (!resInfo.isEmpty()){
			for (ResolveInfo info : resInfo) {
				if (info.activityInfo.packageName.toLowerCase().contains(type) || 
					...
					share.putExtra(Intent.EXTRA_SUBJECT,  "Green Screen Photography");
					share.putExtra(Intent.EXTRA_TEXT,     "This image was sent from the android app Green Screen Photography");
					share.putExtra(Intent.EXTRA_STREAM, Uri.fromFile(new File(s)) ); // Optional, just if you wanna share an image.
			...
			if (!found){
				if (type=="twi") Toast.makeText(getBaseContext(), "Could not find the twitter app", Toast.LENGTH_SHORT).show(); 
				if (type=="face") Toast.makeText(getBaseContext(), "Could not find the facebook app", Toast.LENGTH_SHORT).show(); 
				if (type=="insta") Toast.makeText(getBaseContext(), "Could not find the instagram app", Toast.LENGTH_SHORT).show(); 
				if (type=="pint") Toast.makeText(getBaseContext(), "Could not find the pintrest app", Toast.LENGTH_SHORT).show(); 
				return;
				}
