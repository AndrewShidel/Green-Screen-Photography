...
public class Pic1View extends Activity {
	...
	double ratio,bx;
	int count=0;
	boolean pick=false;boolean shouldmodify=false; boolean cancelmessage=false;
	boolean touched=false;
	float tx1,ty1,tx2,ty2;
	float transX1=0; float transY1=0;float transX2=0; float transY2=0;float transX=0; float transY=0;boolean two=false;
    ...
    public void onCreate(Bundle savedInstanceState) {
	   ...
	   // button1=BitmapFactory.decodeResource(getResources(), R.drawable.button1);
		...
		try{
			...
			Log.d("a","made mutable without crash: "+String.valueOf(bitmap.getWidth()*bitmap.getHeight()));
	...
	/*	try{
		bitmap=Converter.convertToMutable(prebitmap);
			...
			message.cancel();
			 message = Toast.makeText(Pic1View.this,"Image is to large. Try again", Toast.LENGTH_SHORT);
			message.setGravity(Gravity.CENTER, message.getXOffset() / 2, 
							   message.getYOffset() / 2);
			message.show();
	...
	//	bitmap.setHasAlpha(true);
		...
		bitmap.getPixels(backup,0,bitmap.getWidth(),0,0,bitmap.getWidth(),bitmap.getHeight());
	    System.gc();
		showDialog(1);
		...
		switch(id) {
			case 1:
				...
				builder.setMessage(getResources().getString(R.string.dialog1))
					...
					.setNegativeButton(getResources().getString(R.string.click), new DialogInterface.OnClickListener() {
...
//			if (dx>=w || dx<=0-bitmap.getWidth()*scaleFactor || dy>=h || dy<=0-bitmap.getHeight()*scaleFactor){
//				dx=0;
//				dy=0;
//			}
			...
			if (dx>w)dx=-1*(bitmap.getWidth()*scaleFactor);
			if (dx<=0-bitmap.getWidth()*scaleFactor) dx=w;
			if (dy>h) dy=-1*(bitmap.getHeight()*scaleFactor);
			if (dy<=0-bitmap.getHeight()*scaleFactor) dy=h;
			
			canvas.drawColor(Color.BLACK);
			canvas.drawBitmap(back,(w/2)-(back.getWidth()/2),(h/2)-(back.getHeight()/2),null);
		...
		//	dy-=detector.getFocusY()*scaleFactor;
           ...
           // canvas.drawBitmap(bitmap,x+(w/2-bitmap.getWidth()/2),y+(h/2-bitmap.getHeight()/2),null);
			...
			/*Toast message = Toast.makeText(Pic1View.this,String.valueOf(color), Toast.LENGTH_SHORT);
			message.setGravity(Gravity.CENTER, message.getXOffset() / 2, 
							   message.getYOffset() / 2);
			message.show(); // display the Toast*/
			...
			canvas.drawText(String.valueOf((int)(tx1-dx-((w/2)-(bitmap.getWidth()/2))))+" "+String.valueOf((int)(ty1-dy-((h/2)-(bitmap.getHeight()/2)))),0,150,textPaint);
		
			
			canvas.drawText("Threshold: "+String.valueOf(t),(w/2),h-textPaint.getTextSize(),textPaint);
		...
		if (event.getAction()==MotionEvent.ACTION_POINTER_2_DOWN){	
			...
			if (MotionEvent.ACTION_DOWN==event.getAction()){
			 ...
			 if (Math.abs(tx1-bacx)<=bar && Math.abs(ty1-bacy)<=bar){
				//if (ty1>=h-(h/8)){
				    ...
				    bitmap.setPixels(backup,0,bitmap.getWidth(),0,0,bitmap.getWidth(),bitmap.getHeight());
					index=0;
					colorA=new int[20];
					bitmap.getPixels(pixels,0,bitmap.getWidth(),0,0,bitmap.getWidth(),bitmap.getHeight());
				...
				if (Math.abs(tx1-bx)<=w/25 && Math.abs(ty1-v2)<=w/25 && MotionEvent.ACTION_DOWN==event.getAction()){
					touched=true;
					
				}
				if (tx1>=b3x && tx1<=b3x+button3.getWidth() && ty1>=b3y && ty1<=b3y+button3.getWidth()){
					if (source != 1){
					Log.d("a","touched");
					//bitmap.getPixels(pixels,0,bitmap.getWidth(),0,0,bitmap.getWidth(),bitmap.getHeight());
					Log.d("a","got pixels");
					...
					finish();
						...
						Intent intent2 = new Intent(getBaseContext(), done.class);
			...
			if (MotionEvent.ACTION_MOVE==event.getAction() && touched==true){
				...
				t=(int) ((bx-(w/8))/ratio);
				    ...
				    if (message2 != null) message2.cancel();
			      	message2 = Toast.makeText(Pic1View.this,"Proccessing... Please wait", Toast.LENGTH_LONG);
		    ...
		    if (event.getAction()==MotionEvent.ACTION_UP && Math.abs(event.getX()-px)<=pr && Math.abs(event.getY()-py)<=pr){
				if (pick==false){
					pick=true;
			...
			if (MotionEvent.ACTION_UP==event.getAction() && pick){
				...
				color=bitmap.getPixel((int)((tx1/scaleFactor)-(dx/scaleFactor)),(int)((ty1/scaleFactor)-(dy/scaleFactor)));
                colorA[index]=color;
				index++;
		...
		private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
			...
			//	dy-=detector.getScaleFactor()/detector.getFocusY();
				...
				dx = (dx - detector.getFocusX()) * detector.getScaleFactor() + detector.getFocusX();
			    dy = (dy - h + detector.getFocusY()) * detector.getScaleFactor() + h - detector.getFocusY();
				invalidate(); 
				return true; 
			}
		} 
		...
		public float dst(float x1,float x2,float y1,float y2){
			float distance;
			distance=(float)Math.sqrt((Math.abs(Math.pow((x1-x2),2))+Math.abs(Math.pow((y1-y2),2))));
        ...
        public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
			...
			int[] backA;
			int bheight=(int)((float)amount*hRatio);
			...
			back=Bitmap.createBitmap(backA,amount,bheight,Bitmap.Config.ARGB_8888);
			Matrix bm=new Matrix();
			bm.postScale(span,span);
			
		    back=Bitmap.createBitmap(back,0,0,back.getWidth(),back.getHeight(),bm,false);
		
			
			button2=BitmapFactory.decodeResource(getResources(), R.drawable.back);
			...
			Matrix m=new Matrix();
			m.postScale(r,r);
			button2=Bitmap.createBitmap(button2,0,0,(int)bw,(int)bh,m,true);
			bacx=bax+(button2.getWidth()/2);
			bacy=bay+(button2.getHeight()/2);
			
			button3=BitmapFactory.decodeResource(getResources(), R.drawable.next);
			...
			button3=Bitmap.createBitmap(button3,0,0,(int)bw2,(int)bh2,m2,false);
   ...
   public class TutorialThread extends Thread {
	    ...
	    public void modify(int color1){
			...
			try{
				...
				while (cx<width && cy<height){
					...
					g2=Color.green(bp);
					b2=Color.blue(bp);
					if (Math.abs(a-a2)<=t && Math.abs(r-r2)<=t && Math.abs(g-g2)<=t*2 && Math.abs(b-b2)<=t)
					{
        ...
        public void run() {
            ...
            while (_run) {
			    ...
			    if (index>0)modify(colorA[counter-1]);
				counter++;
				}
				cancelmessage=true;
				}
