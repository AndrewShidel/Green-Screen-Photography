...
public class Resize extends Activity {
	...
	float th=0; float th1=0; float th2=0; float focX,focY;
	float rI,rN;
	...
	protected Dialog onCreateDialog(int id) {
		...
		switch(id) {
			...
			case 2:
				...
				builder2.setMessage("Set the image path by going to the desired folder, tapping new, and typing file name")
					.setCancelable(false)
    ...
    class Panel extends SurfaceView implements SurfaceHolder.Callback {
        ...
        public void onDraw(Canvas canvas) {
			...
			if (dx-(pic1.getWidth()/2)>w)dx=-1*(pic1.getWidth()*scaleFactor);
			if (dx-(pic1.getWidth()/2)<=0-pic1.getWidth()*scaleFactor) dx=w;
			if (dy-(pic1.getHeight()/2)>h) dy=-1*(pic1.getHeight()*scaleFactor);
			if (dy-(pic1.getHeight()/2)<=0-pic1.getHeight()*scaleFactor) dy=h;
			...
			if (pic2!=null && temp)
			canvas.drawBitmap(pic2,0-pic2.getWidth()/2,0-pic2.getHeight()/2,null);
			...
			try{
		    if (!zero)canvas.drawBitmap(button3,b3x,b3y,null);
	...
	//		canvas.drawBitmap(bmp2, dx-cx-((pic2.getWidth()*scaleFactor)/2), dy-cy-((pic2.getHeight()*scaleFactor)/2), null);
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			if (MotionEvent.ACTION_DOWN==event.getAction()){
				...
				if (tx1<=b3x+button3.getWidth() && tx1>=b3x && ty1>=b3y && temp){
        ...
        public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
            ...
            if (width/pic1.getWidth()<=height/pic1.getHeight()){
				...
				pic1=Bitmap.createScaledBitmap(pic1,width,(int)(by*ratio),true);
			}else{
				float ratio=h/pic1.getHeight();
				Log.d("a",String.valueOf(ratio));
				pic1=Bitmap.createScaledBitmap(pic1,(int)(bx*ratio),height,true);
			}
			dx=(w/2)-(pic2.getWidth()/2);
			dy=(h/2)-(pic2.getHeight()/2);
			
			button3=BitmapFactory.decodeResource(getResources(), R.drawable.save);
